...
```{r 'P01-Structure-Kite', decorate=TRUE}
str(p01kite)
```

### Import R {.unlisted .unnumbered}

```{r 'P01-CSV-Kite-A', decorate=TRUE, eval=FALSE, ref.label=c('P01-CSV-Kite')}
# #Ref another file chunk
```

### R to Python {.unlisted .unnumbered}

```{r 'P01-R2Y-Kite-Prep', decorate=TRUE}
# #R Matrix | R DataFrame |
p01kite_df_r <- as.data.frame(p01kite, row.names = FALSE)
```

```{python 'P01-R2Y-Kite', decorate=TRUE}
# #R Matrix | R DataFrame | Pandas DataFrame | Copy | NumPy Structured Array | 
p01kite_pd_y = r.p01kite_df_r
pp = p01kite_pd_y.copy()
qq = np.array([tuple(Row) for Row in pp.to_numpy(copy = True)], dtype = list(pp.dtypes.items()))
p01kite_np_y = qq.copy()
p01kite_np_y
...
```{python 'P01-CSV-Kite-Y', decorate=TRUE}
loc_src = "./../data/P01-Kite.csv"
p01kite_y = pd.read_csv(loc_src)
#
# #Drop a Column
pp = p01kite_y.copy()
