...
#### R Dataset {.unlisted .unnumbered}

```{r 'P01-R-iGraph-Kite-Standard', decorate=TRUE}
# #Use standard dataset provided with the igraph package
bb <- make_graph("Krackhardt kite")
# #Assign Letters
V(bb)$name <- LETTERS[1:vcount(bb)]
# #Verify Edgelist between Pacakge Data and CSV
stopifnot(all(identical(attributes(E(bb))$vnames, attributes(E(kite_u))$vnames), 
              identical(attributes(V(bb))$names, attributes(V(kite_u))$names)))
# #Compare Graphs: NOTE: Avoid identical_graphs() as it looks for R internal representation
stopifnot(isomorphic(bb, kite_u))
...
# #Copy
#y_ig_kite = copy.deepcopy(kite_u)

# #Get DataFrame from iGraph
ss = pd.DataFrame(kite_u.get_adjacency().data, columns = kite_u.vs['name'], dtype = np.int32)
assert(pp.equals(ss))

```

#### Python Dataset {.unlisted .unnumbered}

```{python 'P01-Y-iGraph-Kite-Standard', decorate=TRUE}
# #Use standard dataset provided with the igraph package
...
# #Verify
assert(0 == ig.Graph.difference(kite_u, pp).ecount() and 
       0 == ig.Graph.difference(pp, kite_u).ecount() and
       pp.isomorphic(kite_u))
# #Copy
y_ig_kite = copy.deepcopy(pp)

# #Indices
print(y_ig_kite.vs.indices)
print(y_ig_kite.vs['name'])
print(y_ig_kite)

```

#### Python iGraph & NetworkX {.unlisted .unnumbered}

```{python 'P01-Y-iGraph-NetworkX', decorate=TRUE}
pp = copy.deepcopy(y_ig_kite)

# #Convert iGraph to NetworkX
if(False):
    # #This loses the Node Names and Degree 0 Nodes
    print(type(pp))
    qq = pp.get_edgelist()
    print(type(qq))
    ss = nx.Graph(qq)
...
# #Convert iGraph to NetworkX & keep the node names
ss = nx.from_edgelist([(names[x[0]], names[x[1]]) 
                      for names in [pp.vs['name']] 
                      for x in pp.get_edgelist()], nx.Graph())
...
# #Copy
y_nx_kite = copy.deepcopy(ss)

# #Convert NetworkX to iGraph
tt = ig.Graph.TupleList(ss.edges())

# #Verify
assert(tt.isomorphic(pp))

# #Unable to use the syntax for Class Method from_networkx()
#uu = ig.Graph.from_networkx(pp) #AttributeError: 'Graph' object has no attribute 'graph'
#uu = ig.from_networkx(pp) #AttributeError: module 'igraph' has no attribute 'from_networkx'
#pp.from_networkx() #TypeError: Graph.from_networkx() missing 1 required positional argument: 'g'
#pp.from_networkx(pp) #AttributeError: 'Graph' object has no attribute 'graph'
...
if(False): ig.plot(y_ig_kite, layout = pp)

# #Set Visual Style
visual_style = {}
#visual_style["layout"] = y_ig_kite.layout_fruchterman_reingold()
visual_style["layout"] = pp

if(False): ig.plot(y_ig_kite, **visual_style)
...
pp = copy.deepcopy(y_ig_kite)
# #NetworkX
qq = copy.deepcopy(y_nx_kite)

# #iGraph: Dictionary of each Vertex Degree by combining Two Lists
ss = dict(zip(pp.vs['name'], pp.degree(), strict = True))
print(ss)
#ss.keys()
#ss.values()

# #NetworkX: Dictionary of each Vertex Degree 
ss = dict(qq.degree())
print(ss)
# Degree Centrality: Fraction /Normalised
tt = nx.degree_centrality(qq)
{k:round(v, 3) for k, v in tt.items()}
...
if(False):
    ...
    #
    # #Sorted Dictionary - 1
    uu = sorted(ss.items(), reverse = True, key = operator.itemgetter(1))[0:len(ss)]
    print("\n".join(map(lambda x: "{} : {}".format(*reversed(x)), uu)))
    #
    # #Sorted Dictionary - 2
    {k: v for k, v in sorted(ss.items(), reverse = True, key = lambda item: item[1])}
    #
    # #Sorted Dictionary - 3 (0 for Keys, 1 for Values)
    dict(sorted(ss.items(), reverse = True, key = lambda item: item[1]))
...
closeness(bb, mode = "all", normalized = FALSE) |> round(3) #Match with NodeXL
...
# #iGraph: List of Closeness for each node
ss = [round(i, 3) for i in pp.closeness(normalized = False)] #Match with NodeXL
print(ss)
[round(i, 3) for i in pp.closeness(normalized = True)] #Match with NetworkX

# #NetworkX: Dictionary of Closeness (Only Normalised option is available)
tt = nx.closeness_centrality(qq)
{k:round(v, 3) for k, v in tt.items()}
...
bb <- r_kite
#
# #Vector of Betweenness for each node
betweenness(bb) |> round(3)
#
# #Communities
...
# #iGraph: List of Betweenness for each node
ss = [round(i, 3) for i in pp.betweenness()]
print(ss)

# #NetworkX: Dictionary of Betweenness
tt = nx.betweenness_centrality(qq, normalized = False)
# #NOTE: Only the Node Location of E & F has been interchanged, Value is NOT different
[round(v, 3) for k, v in tt.items()]
{k:round(v, 3) for k, v in tt.items()}

if(False): nx.modularity_matrix(qq)
...
bb <- r_kite
# #Eigenvector and Eigenvalue Score
ii <- eigen_centrality(bb, scale = FALSE)$vector |> round(3) #|> sort()
jj <- centr_eigen(bb, scale = FALSE)$vector |> round(3) #|> sort()
kk <- evcent(bb, scale = FALSE)$vector |> round(3) #|> sort()
# #Spectrum might give values reversed during subsequent execution
#ll <- spectrum(bb, which = list(pos = "LA"))$vectors |> round(3) |> as.vector() #|> sort() 
# #Verify
stopifnot(all(identical(unname(ii), jj), identical(ii, kk)))
#
# #Eigenvector of each Vertex
eigen_centrality(bb, scale = FALSE)$vector |> round(3) #|> sort()
eigen_centrality(bb, scale = TRUE)$vector |> round(3) #|> sort()
#
# #Eigenvalue Score
eigen_centrality(bb, scale = FALSE)$value
...
# #iGraph: List of Eigenvalues for each node
[round(i, 3) for i in pp.eigenvector_centrality(scale = True)]
[round(i, 3) for i in pp.eigenvector_centrality(scale = False)] #Match with NetworkX

# #NetworkX: Dictionary of Eigenvalues (No Normalization)
tt = nx.eigenvector_centrality(qq)
{k:round(v, 3) for k, v in tt.items()} 
...
if(TRUE) f_rmExist(aa, away_d, bb, btw, cap_hh, cliq_hh, def_par, dia_hh, e_col_hh, e_w_hh, 
                    far_hh, greed, hh, hh_b, hh_g, hh_w, ii, ii_deg, ii_egn, ii_res, jj, kite_d, 
                    kite_u, kk, lgd_col_hh, lgd_size_hh, lgd_ttl_col_hh, lgd_ttl_size_hh, ll, 
                    loc_bin, loc_png, loc_rds, loc_src, P01I01, P01I02, P01I03, p01r_kite, 
                    p01r_km_1, p01r_km_4, pal_hh, r_kite, size_hh, size_m, ttl_hh, ttl_hh_b, 
                    ttl_hh_g, ttl_hh_w, v_col_hh, v_lbl_col_hh, walk)
```

```{r 'P01-R-Validation', decorate=TRUE, include=FALSE, cache=FALSE}
