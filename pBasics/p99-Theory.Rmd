...
```{definition 'Network'}
A \textcolor{pink}{network} is a collection of things and their relationships to one another. The \textcolor{pink}{'things'} that are connected are called nodes, vertices, or entities. The \textcolor{pink}{connections} between the vertices are called edges, ties, relationships or links.
```
...
```{definition 'Vertex-Edge'}
\textcolor{pink}{Vertex count} is the number of entities in the network while the \textcolor{pink}{edge count} is the number of connections among them.
```
