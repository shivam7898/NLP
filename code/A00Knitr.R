...
if (!("q_start" %in% ls(envir = .GlobalEnv))) {
  # #Compile Start Time
  q_start <- Sys.time()
  knitr::opts_chunk$set(echo = TRUE, include = TRUE, eval = TRUE, collapse = TRUE, width = 100,
                        opts.label = 'twofig', message = FALSE, warning = FALSE, error = FALSE,
  ...
  knitr::knit_hooks$set(engine = function(before, options) {
    ...
    if (before) {
      ...
      } else if (q_engine == "definition") {
          q_def[[options$label]] <<- options$name
      } else if (q_engine == "conjecture") {
          q_cnj[[options$label]] <<- options$name
      } else if (q_engine == "example") {
  ...
  # #Chunk Decoration based on Language Engine: R & Python
